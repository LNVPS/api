...
- **A node can fetch its whole desired state** — `GET /api/v1/node/state` (node token) returns the node's data plane plus `vms[]`: every VM the node should be running, with its shape, its image (URL and checksum, fetched by the node itself), rendered cloud-init, MAC, addresses, and whether LNVPS wants it running. The node makes its machine match — it creates what is missing and **destroys what is absent**. That absence is the only tear-down mechanism that survives LNVPS failing: an API that dies mid-provision leaves nothing to clean up, because the half-built VM is simply not in the next document. The format is node-shaped rather than database-shaped, because the node has no business knowing about users, cost plans or subscriptions, and a wire format mirroring the schema would make every migration a node-compatibility question. Cloud-init arrives rendered, so LNVPS decides what a guest is configured with rather than a node running an older daemon configuring it differently from the rest of the fleet. A deleted VM is absent; a *disabled* one stays and is marked not-running, because removing it would tell the node to destroy the customer's disk, which is a very different thing from "stop billing this".
