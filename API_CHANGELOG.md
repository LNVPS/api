...
- **On-chain referral payouts, batched into one transaction** (issue #184) — referrers can choose a new `on_chain` payout mode and register an on-chain Bitcoin payout address (**mainnet**; regtest also accepted in debug builds for the e2e stack). The automated worker pays **every eligible on-chain referrer in a single send-many transaction**, gated by `min-onchain-payout-sats` under `[referral]` (default 1000; `null` disables). Key behaviours:
  ...
  - **Precise outpoint.** Each payout records its exact `"{txid}:{vout}"` outpoint (rows from one batch share the txid, distinct vouts), decoded from the broadcast transaction.
  - **Unified address field.** The mode-specific `lightning_address` column/field is replaced by a single `address` (its type is implied by `mode`). Exposed on `POST`/`PATCH /api/v1/referral` (accept `address` + `mode: "on_chain"`), the `Referral`/`ReferralPayout` responses (`address`, `fee`, `outpoint`), and the admin referral endpoints. Migrations rename `referral.lightning_address` → `address` and add `referral_payout.outpoint` + `fee`. Requires `payments-rs` 0.7.0 (`OnChainProvider::send_coins` returns the raw transaction).
