...
- **Bulk-extend all non-expired VMs** (issue #96) — new admin endpoint `POST /api/admin/v1/vms/extend-all` extends every currently non-expired VM by a given number of days (1–365) in a single request, e.g. to compensate customers for downtime. Only VMs with a concrete future expiry are extended; deleted, never-paid (`expires = null`), and already-expired VMs are skipped. Each extended VM is logged to its history and re-synced via a `SpawnVm` job, identical to the single-VM `PUT /api/admin/v1/vms/{id}/extend`. Returns a summary `{ extended, skipped, failed }`. This fleet-wide action is gated behind a **new `bulk_update` admin permission** (`AdminAction::BulkUpdate`), granted only to the `super_admin` role by default — a migration adds `virtual_machines::bulk_update` to super_admin, and operators can grant it to custom roles via the roles API.
