...
- **On-chain referral payouts, batched into one transaction** (issue #184) — referrers can now choose a new `on_chain` payout mode and register an on-chain Bitcoin payout address. The automated payout worker pays **every eligible on-chain referrer in a single send-many transaction** (one network fee for the whole batch, absorbed by LNVPS so each referrer receives their exact owed amount), gated by a separate, higher `min-onchain-payout-sats` threshold under `[referral]` (mempool-fee aware; omit to disable — commission still accrues for manual payout). The address must be a **mainnet** address (regtest also accepted in debug builds for the e2e stack). Exposed on the user referral endpoints (`POST`/`PATCH /api/v1/referral` accept `onchain_address` + `mode: "on_chain"`; `Referral` returns `onchain_address`; `ReferralPayout` returns a shared `txid`) and the admin referral endpoints (`onchain_address` on referral info, `txid` on payout info + settable on `PATCH /api/admin/v1/referrals/{id}/payouts/{payout_id}`). A migration adds the `referral.onchain_address` and `referral_payout.txid` columns. Requires `payments-rs` 0.6.0 (new `OnChainProvider::send_coins` send-many API).
