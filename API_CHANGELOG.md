...
- **Managed app deployments — launch app catalog seeds** (part of the App Deployments epic) — ships validated compose definitions for four Nostr-native launch apps (**strfry**, **HAVEN**, **route96 + MariaDB**, a generic **Blossom** server), seeded into the catalog **disabled** (operators review/price before offering) with the resource footprint computed from each compose. Included in the demo-data generator.
