...
- **Managed app deployments — launch app catalog seeds** (part of the App Deployments epic) — ships validated compose definitions for three Nostr-native launch apps, matching each project's actual deployment docs: **strfry** (community `dockurr/strfry`, config-file), **route96 + MariaDB** (`voidic/route96`, YAML config + DB), and a **Blossom** server (`ghcr.io/hzrd149/blossom-server`, YAML config). Seeded into the catalog **disabled** (operators review/price before offering) with the resource footprint computed from each compose. Included in the demo-data generator.
