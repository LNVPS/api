...
- **Compressed OS images (`.xz`, `.zst`, `.gz`, `.bz2`, `.lzo`) now usable on Proxmox** — image URLs pointing at a compressed disk image previously failed: Proxmox stored the compressed bytes under a `.img` name (its own decompression is unreliable / historically ISO-only), so the disk import produced an unbootable VM. The Proxmox host now downloads the compressed artifact as-is, verifies its checksum against the `SHASUMS` entry (which covers the compressed file), then decompresses it on the host over SSH into the final `.img` (via `xz`/`zstd`/`gzip`/`bzip2`/`lzop`, using a temp-file + atomic move so a partial decompression never leaves a truncated image) and removes the compressed source. Supported extensions: `.xz`/`.lzma`, `.zst`/`.zstd`, `.gz`, `.bz2`, `.lzo`. Affects the admin `POST /api/admin/v1/vm_os_images/{id}/download` endpoint and the periodic image check.
