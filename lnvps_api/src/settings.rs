...
pub struct Settings {
    ...
    /// AI support agent served over the live-chat websocket. When unset the
    /// endpoint is present but refuses connections, so the feature can be
...
/// Hypervisor provisioning configuration. Defined in `lnvps_api_common` so any
/// crate that can build a host client can also describe one; re-exported here
/// because it is deserialized as part of [`Settings`].
pub use lnvps_api_common::host::config::{
    FirewallConfig, FirewallPolicy, LibVirtConfig, ProvisionerConfig, ProxmoxConfig, QemuConfig,
    SshConfig,
};

/// Live-chat support agent configuration.
#[cfg(feature = "agent")]
#[derive(Debug, Clone, Deserialize, Serialize)]
#[serde(rename_all = "kebab-case")]
pub struct AgentConfig {
    /// OpenAI-compatible endpoint the agent talks to (Ollama, vLLM, OpenAI...).
    pub openai: lnvps_agent::settings::OpenAiConfig,

    /// Extra instructions appended to the agent's system prompt.
    ///
    /// Deployment-specific guidance (tone, escalation wording, house rules)
    /// that shouldn't be compiled into the binary.
    pub system_prompt: Option<String>,

    /// Maximum characters accepted in a single chat message.
    ///
    /// A long message is a cost and context-window problem before it is
    /// anything else, so it is rejected at the socket rather than forwarded.
    #[serde(default = "default_agent_max_message_chars")]
    pub max_message_chars: usize,

    /// Maximum messages one websocket connection may send.
    ///
    /// Bounds the cost of a single abusive connection. Note this is per
    /// connection, not per user — a cross-connection budget would need shared
...
#[cfg(feature = "agent")]
pub fn default_agent_max_message_chars() -> usize {
    4000
}

/// Default cap on messages per websocket connection.
#[cfg(feature = "agent")]
