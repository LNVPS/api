...
#[get(
    "/api/admin/v1/reports/time-series?<start_date>&<end_date>&<interval>&<company_id>&<currency>"
)]
pub async fn admin_time_series_report(
    ...
    let mut period_aggregations: HashMap<(String, String), (u32, u64, u64, u64, u64)> =
        HashMap::new(); // (period, currency) -> (count, net, tax, base_net, base_tax)
    ...
    for payment in payments {
        ...
        let period_str = match interval.as_str() {
            ...
            "weekly" => {
                ...
                let week_start =
                    payment.created.date_naive() - Duration::days(days_since_monday as i64);
        ...
        if company_info.is_none() {
            company_info = Some((
                payment.company_id,
                payment.company_name.clone(),
                payment.company_base_currency.clone(),
            ));
        }
        ...
        let base_currency = Currency::from_str(&payment.company_base_currency).map_err(|_| {
            anyhow::anyhow!("Invalid base currency: {}", payment.company_base_currency)
        })?;
        ...
        let (base_currency_net, base_currency_tax) = if payment_currency == base_currency {
            ...
            let base_currency_net =
                CurrencyAmount::from_f32(base_currency, base_net_decimal).value();
            ...
            let base_currency_tax =
                CurrencyAmount::from_f32(base_currency, base_tax_decimal).value();
    ...
    let mut period_summaries: Vec<TimeSeriesPeriodSummary> = period_aggregations
        ...
        .map(
            |(
                (period, currency),
                (payment_count, net_total, tax_total, base_currency_net, base_currency_tax),
            )| {
    ...
    let (company_id_val, company_name_val, company_base_currency_val) =
        company_info.unwrap_or((company_id, "Unknown Company".to_string(), "EUR".to_string()));
