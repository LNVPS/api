//! Host detection for LNVPS.
//!
//! Reports `cpu_mfg`, `cpu_arch` and `cpu_features` for host registration.
//! Exposed as a library so the node daemon reports the same facts, detected the
//! same way, as the `lnvps-host-info` binary an operator runs by hand — two
//! implementations would eventually disagree about the same machine.
//!
//! Detection that needs a vendor library is behind a feature: `vaapi` links
//! libva, `nvml` loads NVIDIA's NVML. Both are on by default for the binary and
//! off for callers that must build without either installed, in which case the
//! corresponding [`DetectionResult`] is [`Unsupported`](DetectionResult::Unsupported).
...
// The `unsafe` block below is redundant on current compilers, which treat the
// CPUID intrinsics as safe, but removing it would fail to build on older ones.
// Kept, with the warning silenced, until the MSRV moves past that change.
#[allow(unused_unsafe)]
#[cfg(target_arch = "x86_64")]
pub fn has_sha512_extensions() -> bool {
...
#[cfg(target_os = "linux")]
pub fn is_nested_virt_enabled() -> bool {
    // Intel
...
#[cfg(all(target_os = "linux", feature = "vaapi"))]
pub fn detect_vaapi() -> DetectionResult {
    let mut features = Vec::new();
...
/// Without the `vaapi` feature (or off Linux) there is no libva to ask, so the
/// result is `Unsupported` rather than an empty feature list — "we did not
...
/// Collect everything `lnvps-host-info` reports, in one call.
///
/// Feature lists are sorted and deduplicated here rather than by the caller, so
/// every consumer sees the same canonical ordering.
pub fn host_info() -> HostInfo {
    let mut cpu_features = detect_cpu_features();

    // VA-API (iGPU) features are reported as CPU features.
    let vaapi = detect_vaapi();
    cpu_features.extend(vaapi.features().iter().cloned());
    cpu_features.sort();
    cpu_features.dedup();

    // Discrete GPU (NVIDIA NVENC/NVDEC, AMD VCN/AMF).
